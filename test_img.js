...
let config;
let urls;
if (fs.existsSync('config/backstop_img.json')) {
  config = JSON.parse(fs.readFileSync('config/backstop_img.json'));
  urls = fs.readFileSync('config/urls_img.txt').toString().split("\n");
}
else {
  config = JSON.parse(fs.readFileSync('backstop_img.json'));
  urls = fs.readFileSync('config/urls_img.txt').toString().split("\n");
}
...
let baseURI;
if (process.env.npm_lifecycle_event == 'reference-img') {
